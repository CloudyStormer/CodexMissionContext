...
- 用户明确要求直接迁入当前 E，即使其为 USB/exFAT；未改网络设置。
- 已开始从 `D:\C-Drive-Migration-Staging\CodexHome` 向 `E:\CodexData` 进行只复制的种子传输，并排除实时 SQLite 主库及其 WAL/SHM，计划后续使用 SQLite 一致性备份。
- 传输在 `archived_sessions` 大量数据阶段进入 Windows 无法终止的 I/O 等待；实时 I/O 一度归零，连对 E 的只读目录统计也阻塞。`taskkill` 报告该复制进程没有可终止的运行实例，但 WMI/tasklist 仍显示残留记录，属于外接盘或文件系统的非正常 I/O 状态。
- 此时 `fsutil dirty query E:` 仍显示 Not Dirty，但这不足以证明盘能可靠完成海量文件迁移。**未切换任何持久化路径，未删除或移动 C/D 源数据，也未将 E 的部分副本视为可用结果。**
- 后续只能在 E 恢复正常读写、残留 I/O 任务消失后，从已复制文件断点续传；完整哈希、SQLite 校验和新路径实际启动验证全部通过之前，不得清理 D/C。
