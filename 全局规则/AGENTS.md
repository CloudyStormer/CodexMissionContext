...
- 用户发送四字口令“开始同步”：执行 `check-projects` 的完整同步模式，拉取上下文，刷新当前可访问的本地、远程和账号级线上任务状态，核对项目并集，按 `同步清单/侧边栏布局.md` 的版本协议同步共享项目顺序、项目置顶和共同长期任务置顶，并获取当前设备全部项目仓库的远端最新提交；业务仓库只有工作树干净且可 fast-forward 时才拉取，脏工作树只保留并报告，不 stash、不暂存、不提交、不推送；任何不可达来源必须明确报告。
