...
- 用户发送四字口令“开始同步”：执行 `check-projects` 的完整同步模式，拉取上下文，刷新当前可访问的本地、远程和账号级线上任务状态，核对项目并集，按 `同步清单/侧边栏布局.md` 的版本协议同步共享项目顺序、项目置顶和共同长期任务置顶，并获取当前设备全部项目仓库的远端最新提交；任何不可达来源必须明确报告。
...
- 侧边栏布局只持久化逻辑项目/任务 ID、顺序、置顶状态和版本；不得复制或提交 Codex 原始全局状态。应用顺序和置顶必须使用正式 API 或用户可见界面，禁止直接修改本地数据库或全局状态文件。
