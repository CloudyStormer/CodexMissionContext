...
**问：拿到 `Response` 是否代表完整答案已到？**
答：不代表。响应头和 body 读取入口已到，响应体仍可持续传输。

**问：为什么一个 Fetch 会多次处理数据？**
答：Fetch 只执行一次，同一个循环多次调用 `reader.read()`。

**问：为什么残缺 JSON 不会马上 `JSON.parse()`？**
答：没有 `\n\n` 时，最后一段被 `frames.pop()` 留在 `pendingText`，不会进入解析循环。

**问：第三条 SSE 中断会影响前两条吗？**
答：不会。每条 `delta` 的 JSON 都独立完整，已显示的前两条仍保留。

**问：网络恢复后会自动从断点继续吗？**
答：当前 Fetch 实现不会；需要事件 ID、持久化和重连协议。
